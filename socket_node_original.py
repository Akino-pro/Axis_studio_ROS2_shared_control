...
def _init_mqtt():
    cli = mqtt.Client(client_id="kentucky_publisher")
    cli.username_pw_set(USER, PASS)
    cli.tls_set()               # use system CAs; for self-signed pass paths here
    cli.connect(BROKER, PORT, keepalive=30)
    cli.loop_start()            # runs network loop in background thread
    return cli
...
class SocketNode(Node):
    ...
    def _log_positions_if_changed(
        ...
        """
        Append one line  t  ex ey ez  wx wy wz  px py pz
        but only when the 9-tuple (rounded to 6 dp) differs from the last write.
        """
        vec = np.hstack([elbow, wrist, pump]).round(6)   # shape (9,)

        # skip if identical to last saved
        if self._last_saved_pos is not None and np.array_equal(vec, self._last_saved_pos):
            return

        t = self.get_clock().now().nanoseconds * 1e-9
        line = " ".join([f"{t:.6f}"] + [f"{v:.6f}" for v in vec])
        self._pos_log_fh.write(line + "\n")
        self._last_saved_pos = vec.copy()     

    def destroy_node(self):
        try:
            self._pos_log_fh.close()
        finally:
            super().destroy_node()
    ...
    def start_receiver(self):
        ...
        try:
            while True:
                ...
                self._log_positions_if_changed(position_array[1], position_array[2], position_array[4])
    ...
    def publish_joint_state(self):
		...
		"position": [float(f"{x:.6f}") for x in ik_results_to_use[0:7]]
        }
      
        mqtt_cli.publish(TOPIC,
